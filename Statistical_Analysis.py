...
class Stat_Analysis():
	...
	def article_classifier(self):
		...
		#self.make_feature_graph(X_dev[:,0:1], "developmentSetLabels.dat")
		#exit()
		#self.make_feature_graph(X[:,0:1],"balancedTrainingDataLabels.dat")
		#exit()
		...
		'''
		testArticles, trainArticles = preprocessData.main()
		...
		for i in range(len(y_dev)):
			...
			if(y_dev[i] == 1):
				print(i+1,y_dev[i],trainArticles[i].numberOfSentences,trainArticles[i].avgSentLength,prob,lab,X_dev[i])
...
data_object = Load_Data("balancedTrainingData.dat","balancedTrainingDataLabels.dat")
